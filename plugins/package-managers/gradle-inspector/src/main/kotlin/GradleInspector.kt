...
class GradleInspector(
    ...
    private fun extractInitScript(): File {
        ...
        val modelJar = extractResource("/gradle-model.jar", createOrtTempFile(prefix = "model", suffix = ".jar"))
        ...
        val initScriptText = javaClass.getResource("/init.gradle").readText()
            ...
            .replace("<REPLACE_MODEL_JAR>", modelJar.invariantSeparatorsPath)
    ...
    private fun Collection<OrtDependency>.toPackageRefs(): SortedSet<PackageReference> =
        mapTo(sortedSetOf()) {
            val (type, linkage) = if (it.localPath != null) {
                /*managerName*/"Gradle" to PackageLinkage.PROJECT_DYNAMIC
            } else {
                "Maven" to PackageLinkage.DYNAMIC
            }

            PackageReference(
                id = Identifier(type, it.groupId, it.artifactId, it.version),
                linkage = linkage,
                dependencies = it.dependencies.toPackageRefs()
            )
        }
